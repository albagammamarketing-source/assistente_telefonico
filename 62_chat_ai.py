...
def get_total_price(
    ...
    if camera_key not in prezzi.columns:
        raise ValueError(
            f"Nel foglio PREZZI manca la colonna camera: {camera_key}"
        )

    prezzi["data"] = pd.to_datetime(
        prezzi["data"],
        errors="coerce"
    ).dt.date
    ...
    while current < check_out:
        ...
        if row.empty:
            raise ValueError(
                f"Prezzo mancante per {camera_key} in data {current}"
            )
        ...
        if pd.isna(price):
            raise ValueError(
                f"Prezzo vuoto per {camera_key} in data {current}"
            )
...
def check_availability(req: AvailabilityRequest):
    ...
    if check_out <= check_in:
        ...
        return {
            "available": False,
            "message": (
                "La data di check-out deve essere "
                "successiva al check-in."
            ),
            "room_name": "",
            "total_price": 0
        }
    ...
    try:
        ...
        return {
            "available": False,
            "message": (
                f"Errore nel caricamento del foglio "
                f"CAMERE_CONFIG: {str(e)}"
            ),
            "room_name": "",
            "total_price": 0
        }
    ...
    for col in required_cols:
        ...
        if col not in camere.columns:
            ...
            return {
                "available": False,
                "message": (
                    f"Nel foglio CAMERE_CONFIG "
                    f"manca la colonna: {col}"
                ),
                "room_name": "",
                "total_price": 0
            }
    ...
    camere_struttura = camere[
        ...
        .astype(str)
        .str.lower()
        .str.contains(struttura_request, na=False)
    ...
    if camere_struttura.empty:
        ...
        return {
            "available": False,
            "message": (
                f"Non ho trovato la struttura richiesta: "
                f"{req.structure}. "
                f"Puoi ripetere il nome?"
            ),
            "room_name": "",
            "total_price": 0
        }
    ...
    if not disponibili:
        ...
        return {
            "available": False,
            "message": (
                f"Mi dispiace, non risultano disponibilità "
                f"dal {req.check_in} al {req.check_out} "
                f"per {req.guests} ospiti."
            ),
            "room_name": "",
            "total_price": 0
        }
    ...
    return {
        ...
        "message": (
            f"Sì, abbiamo disponibilità per "
            f"{migliore['room_name']}. "
            f"Il prezzo totale dal "
            f"{req.check_in} al "
            f"{req.check_out} è "
            f"{migliore['total_price']} euro."
        ),
